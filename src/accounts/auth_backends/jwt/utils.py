...
def _generate_refresh_token(base_payload):
    return {"token_type": "refresh", "exp": REFRESH_TOKEN_EXPIRY, **base_payload}


def _generate_access_token(base_payload):
    return {"token_type": "access", "exp": ACCESS_TOKEN_EXPIRY, **base_payload}
...
def generate_tokens(username):
    ...
    return (
        base_payload["jti"],
        encode_payload(access_payload),
        encode_payload(refresh_payload),
    )
